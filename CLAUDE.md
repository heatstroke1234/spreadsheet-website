...
- **Workload Identity Federation (WIF)**, used in production — if `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, and `ANTHROPIC_SERVICE_ACCOUNT_ID` are all present, the client authenticates via `new Anthropic({ apiKey: null, credentials: federationAccessTokenProvider })` — a `credentials` provider that exchanges an AWS STS-issued OIDC token (`sts:GetWebIdentityToken`, requires "Outbound web identity federation" enabled at the AWS account level) for a short-lived `sk-ant-oat01-...` token via Anthropic's `/v1/oauth/token` federation endpoint — no long-lived API key ever touches the deployed environment. `oidcFederationProvider()` (from `@anthropic-ai/sdk/lib/credentials/oidc-federation`) performs a fresh token exchange on every call by design (federation grants don't return a refresh token) — it's wrapped in a `TokenCache` (`@anthropic-ai/sdk/lib/credentials/token-cache`) so a warm Lambda reuses the cached token instead of re-exchanging on every chat message; `sts`/`tokenCache` are module-scoped for the same reason. `ANTHROPIC_WORKSPACE_ID` is optional (only needed if the federation rule spans multiple workspaces); the Terraform variable defaults to `""`, which the SDK correctly treats as omitted rather than as a literal empty workspace id. The IAM role that Amplify's SSR Lambda assumes (`aws_iam_role.amplify_compute` in `infra/amplify.tf`, wired via `compute_role_arn` on `aws_amplify_app.main`) needs only `sts:GetWebIdentityToken` — nothing else. **`apiKey: null` is load-bearing, not defensive boilerplate** — see the bug below.
- **Static API key**, local/Docker only — falls back to plain `new Anthropic()` (reads `ANTHROPIC_API_KEY` from the environment) whenever the federation vars aren't all present, which is how `.env.local` and Docker runs keep working unchanged (neither has an Amplify compute role to federate through, and the compute role's trust policy only allows `amplify.amazonaws.com` to assume it — not even an account admin can assume it locally to test WIF end-to-end). `ANTHROPIC_API_KEY` was removed from `infra/amplify.tf`/`variables.tf` once WIF was confirmed genuinely working in production (2026-08-05) — see the Claude Console for revoking the old key value once nothing else references it, since Terraform can't do that part.

**A masking bug that made WIF look done a full deploy before it actually was.** The first "successful" production test (clean response, zero errors, our own `console.log` printing `auth path: WIF`) wasn't actually using WIF at all. The Anthropic SDK's constructor silently backfills `apiKey` from `readEnv('ANTHROPIC_API_KEY')` whenever the `apiKey` constructor option is left `undefined` — and a resolved `apiKey` wins over an explicitly-passed `credentials` provider, which then never even gets consulted (confirmed directly in `node_modules/@anthropic-ai/sdk/src/client.ts`: "apiKey/authToken win over credentials/config/profile"). `ANTHROPIC_API_KEY` was still present in `.env.production` at that point (the Terraform removal hadn't shipped yet), so the client silently authenticated with the static key the whole time — our diagnostic log only reflects *our own* `hasFederationConfig` branch decision, not what the SDK actually did with it, so it confidently reported "WIF" regardless. The masking only lifted once `ANTHROPIC_API_KEY` was fully removed from the build, which forced `credentials` to actually run for the first time — immediately surfacing a real, independent problem that had been sitting there the whole time: the Federation Rule's `aws_account` claim condition was checking a claim nested under the `https://sts.amazonaws.com/` namespace, which AWS IAM Identity Center only populates for SSO-originated sessions (confirmed by decoding a real token from a personal `aws sts get-web-identity-token` call) — never present on the Amplify compute role's plain service-assumed session (`userIdentity.type: AWSService, invokedBy: amplify.amazonaws.com` in CloudTrail), so every real request failed with `match_claim_absent`. Fixed on both sides: the rule's `aws_account` condition was removed from the Claude Console (redundant anyway — the subject pattern already pins the exact role ARN, which includes the account id), and `createAnthropicClient()` now passes `apiKey: null` explicitly instead of leaving it `undefined`, so an ambient `ANTHROPIC_API_KEY` can never again silently pre-empt `credentials`. Re-verified for real afterward via the same auth-path log *plus* a matching `GetWebIdentityToken` CloudTrail event from the compute role at the same timestamp — that second check is the one that actually proves something, since the log line alone had already been shown to lie.
